...
import top.zxqs.utils.reflect.ReflectUtils;
...
public class ExcelUtil<T> {
    ...
    /**
     * 注解列表，Object[0] 是成员变量，Object[1] 是成员变量上的注解，也就是 Excel 注解
     */
    ...
    public void writeSheet() {
        // 取出一共有多少个sheet.这里一个 sheet 初始值是 65536 行，超过这个值就再创建一个sheet
                ...
                for (Map.Entry<Integer, Object[]> entry : fieldsMap.entrySet()) {
                    ...
                    if (StringUtils.isNotNull(fieldType)) {
                        ...
                        if (StringUtils.isNotEmpty(attr.targetAttr())) {
                            ...
                            // 反向解析值 男=0,女=1,未知=2,导入是Excel是文字，保存到表中需要是数字
                        ...
                        // 通过反射调用 entity 的 settter 方法给 entity 中的属性进行赋值
    ...
    /**
     * 创建工作表，数据行数大于 65536，在创建一个 sheet 相当于用两个工作表存数据
     *
    ...
    public void setDataValidation(Excel attr, Row row, int column) {
        ...
        // 设置鼠标点击显示的提示信息 prompt 是 Excel 注解中的属性
    ...
    /**
     *
     * 初始化Excel，主要构建工作簿和工作表
     * @param list           表格的数据
     * @param sheetName      工作表的名称
     * @param title          标题
     * @param type           段类型（0：导出导入；1：仅导出(只在导出是显示字段)；2：仅导入(只在导入是显示字段)）
     */
